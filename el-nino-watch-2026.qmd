...
subtitle: "Tracking coral bleaching risk across seven reef sites during the 2026 Super El Niño period"
author: "Frank Velez"
date: today
date-format: "MMMM D, YYYY — HH:mm UTC"
categories: [coral reefs, NOAA CRW, DHW, El Niño, live monitoring]
image: ../images/el-nino-watch-preview.png
description: "Live thermal stress monitoring across seven reef sites during the 2026 Super El Niño period, using NOAA Coral Reef Watch DHW data. Updated daily."
freeze: false
execute:
  echo: fenced
  warning: false
...
# -- Output directories -------------------------------------------------------
PROJECT_ROOT = pathlib.Path("/Users/renatus/code/el-nino-watch-2026")
...
def dhw_to_alert(dhw, hotspot=None):
    """
    Map DHW and HotSpot values to a NOAA CRW bleaching alert level string.

    No Stress is determined by HotSpot (SST minus MMM baseline) <= 0, not by
    DHW being negative — DHW is a clamped accumulator and is never negative in
    practice. The 5-level DHW thresholds apply once HotSpot is positive.
    ...
    if pd.isna(dhw):
        return "No Stress"
    if hotspot is not None and not pd.isna(hotspot) and hotspot <= 0:
        return "No Stress"
    if dhw < 4:
...
def fetch_oni():
    ...
    Returns (df, source) where source is "live", "cache", or None.
    Never raises — all network failures degrade gracefully to the
    on-disk cache, and return (None, None) if no cache exists.
    """
    SEASON_TO_MONTH = {
    ...
    cache_path = CACHE_DIR / "oni.ascii.txt"
    ua = (
    ...
    for attempt in range(3):
        try:
            resp = requests.get(ONI_URL, timeout=30, headers={"User-Agent": ua})
...
for name, df in site_data.items():
    last = df.iloc[-1]
    alert = dhw_to_alert(last["dhw"], last.get("hotspot"))
    rows.append({
        "Site":              name,
...
rolling 84-day window. No Stress means SST is at or below the site's MMM bleaching
baseline (HotSpot ≤ 0); Watch begins once SST exceeds it (HotSpot > 0, DHW < 4).
The NOAA CRW 5-level alert system (expanded December 2023):
Warning < 8, Alert Level 1 < 12, Alert Level 2 < 16, Alert Level 3 < 20,
...
## 2026 SST anomaly time series

DHW measures accumulated heat stress and stays near zero until SST sustains more than 1°C above the site's bleaching baseline — it is a lagging indicator. SST anomaly is the departure from the long-term average and responds immediately to warming, making it a leading indicator that shows heat building earlier in the season.

```{python}
# =============================================================================
# SST anomaly time series panel plot
# =============================================================================
sites_to_plot_anom = list(site_data.keys())
n_anom = len(sites_to_plot_anom)

if n_anom == 0:
    print("No data to plot.")
else:
    ncols_a = 2
    nrows_a = (n_anom + ncols_a - 1) // ncols_a
    fig_a, axes_a = plt.subplots(nrows_a, ncols_a, figsize=(14, nrows_a * 3.2),
                                 sharey=False, constrained_layout=True)
    axes_flat_a = axes_a.flat if n_anom > 1 else [axes_a]

    for ax, name in zip(axes_flat_a, sites_to_plot_anom):
        df = site_data[name]
        vals = df["sst_anom"]
        dates = df["date"]
        ...
        ax.fill_between(dates, vals, 0,
                        where=(vals >= 0), color="#EF5350", alpha=0.35, zorder=1)
        ax.fill_between(dates, vals, 0,
                        where=(vals < 0),  color="#42A5F5", alpha=0.35, zorder=1)
        ax.plot(dates, vals, color="#424242", linewidth=1.4, zorder=3)
    ...
    for i in range(n_anom, nrows_a * ncols_a):
        axes_a.flat[i].set_visible(False)

    fig_a.suptitle(f"2026 YTD SST Anomaly — 7 Reef Sites\n"
                   f"NOAA Coral Reef Watch v3.1  |  as of {fetch_stamp}",
                   fontsize=11, y=1.01)

    plt.savefig(OUTPUT_DIR / "sst_anom_panel_2026.png", dpi=150, bbox_inches="tight")
    plt.show()
```

SST anomaly is the departure from the long-term average for the date and is distinct from DHW (accumulated thermal stress) — it flags heat building, not bleaching underway.
...
# Fetch ONI — returns (df, source) where source is "live", "cache", or None
oni_df, oni_source = fetch_oni()
...
if oni_source == "live":
    oni_note = "ONI source: NOAA Climate Prediction Center"
elif oni_source == "cache":
    oni_note = "ONI: last cached fetch — live CPC data unavailable this render"
else:
    oni_note = "ONI data unavailable for this render"
...
if oni_source == "cache":
    print("Note: ONI bars shown above reflect a cached last-known-good fetch — live CPC data was unavailable during this render.")
elif oni_source is None:
    print("Note: ONI data was unavailable for this render. Bars and El Niño/La Niña threshold lines are not shown.")
```
